...
import { Code, Copy, Check, ArrowLeft, Search, AlertTriangle } from 'lucide-react';
            ...
            <div className="max-w-5xl mx-auto px-4 sm:px-6 py-12">
                ...
                <div className="mb-8 p-5 rounded-xl border border-white/10 bg-white/[0.02]">
                    ...
                    <div className="space-y-4">
                        <div>
                            <label className="block text-sm text-gray-400 mb-2">Enter your payload</label>
                            <textarea
                                value={customPayload}
                            ...
                            <div className="grid sm:grid-cols-3 gap-3">
                                <div className="p-3 rounded-lg bg-black/30 border border-white/5">
                                    <div className="flex items-center justify-between mb-2">
                                        <span className="text-xs text-gray-500">URL Encoded</span>
                                        <button
                                            onClick={() => copyToClipboard('url', encodePayload(customPayload, 'url'))}
                                            className="p-1 hover:bg-white/10 rounded transition-colors"
                                        >
                                            {copiedId === 'url' ? <Check className="w-3 h-3 text-green-500" /> : <Copy className="w-3 h-3 text-gray-500" />}
                                        </button>
                                    </div>
                                    <code className="text-xs text-orange-400 break-all">{encodePayload(customPayload, 'url')}</code>
                                </div>
                                <div className="p-3 rounded-lg bg-black/30 border border-white/5">
                                    <div className="flex items-center justify-between mb-2">
                                        <span className="text-xs text-gray-500">HTML Encoded</span>
                                        <button
                                            onClick={() => copyToClipboard('html', encodePayload(customPayload, 'html'))}
                                            className="p-1 hover:bg-white/10 rounded transition-colors"
                                        >
                                            {copiedId === 'html' ? <Check className="w-3 h-3 text-green-500" /> : <Copy className="w-3 h-3 text-gray-500" />}
                                        </button>
                                    </div>
                                    <code className="text-xs text-orange-400 break-all">{encodePayload(customPayload, 'html')}</code>
                                </div>
                                <div className="p-3 rounded-lg bg-black/30 border border-white/5">
                                    <div className="flex items-center justify-between mb-2">
                                        <span className="text-xs text-gray-500">Base64 Encoded</span>
                                        <button
                                            onClick={() => copyToClipboard('base64', encodePayload(customPayload, 'base64'))}
                                            className="p-1 hover:bg-white/10 rounded transition-colors"
                                        >
                                            {copiedId === 'base64' ? <Check className="w-3 h-3 text-green-500" /> : <Copy className="w-3 h-3 text-gray-500" />}
                                        </button>
                                    </div>
                                    <code className="text-xs text-orange-400 break-all">{encodePayload(customPayload, 'base64')}</code>
                                </div>
                            </div>
                        )}
                    </div>
                        ...
                        >
                            <div className="flex items-start justify-between gap-4">
                                <div className="flex-1 min-w-0">
                                    ...
                                    <code className="block text-sm text-orange-400 bg-black/50 px-3 py-2 rounded-lg overflow-x-auto">
                                        {p.payload}
                                    </code>
                                </div>
                                <button
